...
// now respected on pipes as of kernel version 5.1+.
...
TEST_P(BlockingRead, Splice) {
  ...
  if (param.skip_on_old_linux && !IsRunningOnGvisor()) {
    auto version = ASSERT_NO_ERRNO_AND_VALUE(GetKernelVersion());
    SKIP_IF(version.major < 5 || (version.major == 5 && version.minor < 1));
  }

  SKIP_IF((param.input_flags || param.output_flags) && IsRunningOnGvisor());
